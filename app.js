...
(function () {
  ...
  window.KasirApp = {
    ...
    viewReceiptModal: (id) => {
      ...
      // Masukkan kode ini setiap kali perangkat ingin mengambil data dari server (menyertakan token)
      ...
      // Masukkan kode ini di bagian JavaScript Anda saat login berhasil (menerima token dari backend)
      ...
      app.post('/login', async (req, res) => {
        ...
        if (user) {
          // Buat token unik untuk perangkat ini
          const sessionToken = generateUniqueToken();

          // Simpan ke database (perangkat baru ditambahkan, perangkat lama tetap aman)
          await db.query(
            'INSERT INTO user_sessions (user_id, device_token, device_name) VALUES (?, ?, ?)',
            [user.id, sessionToken, deviceInfo]
          );
